...
output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    # output MSA file
    fn_out <- file.path(dir_output, paste0(busco, ".fa"))
    fn_out_aligned <- file.path(dir_output, paste0(busco, "_aligned.fa"))
    ...
    for (read in ls_prefix) {
        file_busco_read <- file.path(dir_busco_read, read, busco_read_dir_mid, psate0(busco, ".faa"))
...
foreach (busco = shared_busco) %dopar% {
    ...
    fn_fasta <- file.path(dir_output, paste0(busco, "_aligned.fa"))
    fn_treefile <- file.path(dir_output, paste0(busco, "_aligned.fa.treefile"))
...
foreach (busco = shared_busco) %dopar% {
    ...
    # input file
    fn_treefile <- file.path(dir_busco_tree, busco, paste0(busco, "_aligned.fa.treefile"))
    ...
    fn_treeshrink_tree <- file.path(dir_output, paste0(prefix, ".treefile"))
...
# iterate over shared BUSCOs
output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    dir_output <- file.path(dir_busco_tree, busco, "trimal")
    if (!dir.exists(dir_output)) {
        dir.create(dir_output, recursive=T)
    }
    
    # output MSA file
    fn_out_aligned <- file.path(dir_busco_tree, busco, paste0(busco, "_aligned.fa"))
    fn_out_trimal <- file.path(dir_output, paste0(busco, "_aligned_trimal.fa"))
    ...
    # run IQ-TREE2
    fn_treefile <- file.path(dir_output, paste0(busco, "_aligned_trimal.fa.treefile"))
    if (!file.exists(fn_treefile) || params$redo) {
        # run IQ-Tree2
