...
if (!file.exists(fn_mapped_reads_busco) || params$redo) {
    ...
    if (length(ls_busco_read_done) > 0) {
        log_msg <- c(paste0("Available mapped sequence BUSCO runs (", length(ls_busco_read_done), "/", length(ls_prefix), ")"), ls_busco_read_done, "")
...
```{r, include=is_busco_metadata, eval=is_busco_metadata}
...
```{r check-busco-read, include=FALSE, eval=is_busco_read}
...
```{r check-busco-read-common, include=FALSE, eval=is_busco_metadata}
# output data.table
...
```{r check-busco-read-coverage, include=FALSE}
# iterate over mapped reads
