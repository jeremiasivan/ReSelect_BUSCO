...
lapply(list(dir_busco_lineage, dir_busco_ref, dir_busco_read, dir_busco_tree), function(x){
    if(!dir.exists(x)) dir.create(x, recursive=T)})

# optional output directories
dir_busco_trimal <- file.path(dir_busco, "trees", "trimal")
if (!dir.exists(dir_busco_trimal) && params$run_trimal) {
    dir.create(dir_busco_trimal, recursive=T)
}

dir_busco_treeshrink <- file.path(dir_busco, "trees", "unfiltered_treeshrink")
if (!dir.exists(dir_busco_treeshrink) && params$run_treeshrink) {
    dir.create(dir_busco_treeshrink, recursive=T)
}
...
if (length(ls_busco_ref_done) > 0) {
    log_msg <- c(paste0("Available reference BUSCO runs (", length(ls_busco_ref_done), "/", length(ls_refseq), ")"), ls_busco_ref_done, "")
...
```{r check-busco-ref, include=FALSE, eval=is_busco_ref}
...
```{r check-busco-common, include=FALSE}
# initiate variable
shared_busco <- c()
