...
ls_refseq <- unique(df_refs$species)
ls_shortreads <- unique(df_reads$species)
...
if (length(ls_readmap_done) > 0) {
    log_msg <- c(paste0("Available mapped reads (", length(ls_readmap_done), "/", length(ls_prefix), ")"), ls_readmap_done, "")
    readr::write_lines(log_msg, file=fn_log, append=T)
}
...
if (length(ls_variant_done) > 0) {
    log_msg <- c(paste0("Available VCF files (", length(ls_variant_done), "/", length(ls_prefix), ")"), ls_variant_done, "")
    readr::write_lines(log_msg, file=fn_log, append=T)
}
