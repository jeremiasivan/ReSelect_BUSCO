...
for (i in 1:nrow(df_mashsum)) {
    file_sam <- paste0(dir_readmap, df_mashsum$reads_accession[i], ".sam")
    ...
    ls_dir_reads <- strsplit(df_mashsum$reads_dir[i], split="/")
    dir_reads <- paste(ls_dir_reads[-length(ls_dir_reads)], collapse="/")

    ls_fastq <- list.files(dir_reads, pattern="*.fastq", recursive=F)
