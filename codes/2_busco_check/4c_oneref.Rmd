---
title: "Generate metadata from one reference"
---

## Generate metadata from one reference
...
for (i in 1:length(ls_refseq)) {
    ...
    ls_output <- foreach(busco = shared_busco, .combine='rbind') %dopar% {
        ...
        for (read in ls_shortreads_busco) {
            ...
            closest_ref <- f_check_closest_ref(genome_dist, read_ref_id, ref)

            # update data.table
            df_temp <- rbind(df_temp, data.table::data.table(busco=busco, read=read, ref=ref, genome_dist=closest_ref$dist))
        }
