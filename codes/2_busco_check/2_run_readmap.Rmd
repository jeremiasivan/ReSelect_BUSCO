...
dir_check_shortreads <- paste0(getwd(), "/busco_check/short_reads/")
...
f_write_log(fn_log=fn_log, msg=c("", "---------- Data Conversion ---------"))
...
for (ref in ls_refseq) {
    ...
    foreach (read = ls_shortreads) %dopar% {
        ...
        # check if file exists
        file_fas <- paste0(dir_output, prefix, ".fa")
        if (file.exists(file_fas) && !params$redo) {
            # check if file is not empty
            if (file.size(file_fas) != 0L) {
                log4r::warn(fn_logger, paste0("File found: FASTA file for ", prefix, ". Skipped."))
                return(NULL)
            }
        }
