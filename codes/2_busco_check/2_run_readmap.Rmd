...
if (params$redo) {
    ...
    for (read in ls_shortreads) {
        ...
        file_fastq <- paste0(dir_check_shortreads, read, ".collapsed.truncated")
...
if (params$redo) {
    ...
    for (prefix in ls_prefix) {
        ...
        if (all(file.exists(file_bam, file_vcf, file_fas))) {
...
# output log file for available converted files
if (length(ls_convert_done) > 0) {
    ls_convert_done <- c(paste0("Available converted files (", length(ls_convert_done), "/", length(ls_prefix), ")"), ls_convert_done, "")
...
f_write_log(fn_log=fn_log, msg=paste("Running Samtools and Bcftools using", thread_samtools, "threads"))
...
foreach (prefix = ls_convert_run) %dopar% {
    ...
    log4r::info(fn_logger, paste0("File created: BAM, VCF, and FASTA files for ", prefix, "."))
}
