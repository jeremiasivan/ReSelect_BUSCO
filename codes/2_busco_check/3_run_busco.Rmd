...
foreach (ref = ls_refseq) %dopar% {
    ...
    ls_filenames <- unlist(strsplit(file_ref, split="/"))

    file_metaeuk <- paste0(dir_busco_ref, ref, "/run_", params$busco_lineage, "/metaeuk_output/initial_results/", ls_filenames[length(ls_filenames)], ".fas")
    if (is_gff3_run) {
