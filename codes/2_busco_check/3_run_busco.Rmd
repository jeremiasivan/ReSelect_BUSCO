...
foreach (ref = ls_refseq) %dopar% {
    ...
    # check if Metaeuk GFF3 exists
    file_gff <- paste0(dir_busco_ref, ref, "/", ref, ".gff3")
    if (file.exists(file_gff) && !params$redo) {
        if (file.size(file_gff) != 0L) {
            log4r::info(fn_logger, paste0("File found: BUSCO GFF3 file for ", ref, ". Skipped."))
            is_gff3_run <- FALSE
        }
    }

    # convert Metaeuk to GFF3
    file_refname <- list.files(dir_ref, pattern="*.fna$", recursive=F, full.names=F)[1]

    file_metaeuk <- paste0(dir_busco_ref, ref, "/run_", params$busco_lineage, "/metaeuk_output/initial_results/", file_refname, ".fas")
    if (is_gff3_run) {
        cmd_metaeuk2gff <- paste("python", params$exe_metaeuk2gff, "-f", file_metaeuk, "-g", file_gff)
        system(cmd_metaeuk2gff)

        log4r::info(fn_logger, paste0("File created: BUSCO GFF3 file for ", ref, "."))
    }
...
for (ref in ls_refseq) {
    ...
    file_gff <- paste0(dir_busco_ref, ref, "/", ref, ".gff3")
    if (!all(file.exists(file_fas, file_gff))) {
        log4r::warn(fn_logger, paste0("File not found: FASTA or GFF3 file for ", ref, ". Skipped."))
        next
    }

    # extract FASTA based on GFF3
    file_output_fas <- paste0(dir_busco_ref, ref, "/", ref, ".fa")
    if (file.exists(file_output_fas) && !params$redo) {
        log4r::warn(fn_logger, paste0("File found: BUSCO sequences for ", ref, ". Skipped."))
    } else {
        f_extract_fasta_from_gff(file_fas, file_output_fas, file_gff, params$exe_gffread)
    }

    # open BUSCO GFF file
    df_gff <- data.table::fread(file_gff, header=FALSE, select=1:9)
    data.table::setnames(df_gff, c("seqname", "source", "feature", "start", "end", "score", "strand", "frame", "attribute"))

    # read BUSCO FASTA sequences
    all_seqs <- Biostrings::readDNAStringSet(file_output_fas)
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        # extract BUSCO
        errmsg <- f_extract_busco(busco, busco_header, df_gff, all_seqs, file_out, ref)
        if (!is.null(errmsg)) {
            f_write_log(fn_log=fn_log, msg=errmsg)
        }
    }
...
for (prefix in ls_prefix) {
    ...
    file_gff <- paste0(dir_busco_ref, ref, "/", ref, ".gff3")
    if (!all(file.exists(file_fas, file_gff))) {
        log4r::warn(fn_logger, paste0("File not found: FASTA or GFF3 file for ", prefix, ". Skipped."))
        next
    }

    # extract FASTA based on GFF3
    file_output_fas <- paste0(dir_busco_out, prefix, ".fa")
    if (file.exists(file_output_fas) && !params$redo) {
        log4r::warn(fn_logger, paste0("File found: BUSCO sequences for ", prefix, ". Skipped."))
    } else {
        f_extract_fasta_from_gff(file_fas, file_output_fas, file_gff, params$exe_gffread)
    }
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        # extract BUSCO
        errmsg <- f_extract_busco(busco, busco_header, df_gff, all_seqs, file_out, prefix)
        if (!is.null(errmsg)) {
            f_write_log(fn_log=fn_log, msg=errmsg)
        }
    }
    ...
    for (ref in ls_refseq) {
        ...
        if (!is_monophyletic) {
            if (length(n_shortreads)+1 > length(ls_taxa)) {
                is_monophyletic <- "INCOMPLETE"
            } else if (length(n_shortreads)+1 < length(ls_taxa)) {
                is_monophyletic <- "DUPLICATE"
            }
        }
...
# df_output <- data.table::transpose(df_output, keep.names = "refseq", make.names = "busco")
