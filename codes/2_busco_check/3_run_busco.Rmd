...
ls_species_name <- c()
ls_refseq_species <- NULL
ls_shortreads_species <- NULL

# iterate over reference sequences
for (ref in ls_refseq) {
    ls_species_name <- unlist(strsplit(f_extract_species_from_assembly(params$exe_efetch, ref), split=" "))
    species_name <- paste0(ls_species_name[1],"_",ls_species_name[2])

    ls_species_name[ref] <- species_name
    ls_refseq_species <- c(ls_refseq_species, species_name)
}

# iterate over short reads
for (read in ls_shortreads) {
    ls_species_name <- unlist(strsplit(f_extract_species_from_reads(params$exe_efetch, read), split=" "))
    species_name <- paste0(ls_species_name[1],"_",ls_species_name[2])

    ls_species_name[read] <- species_name
    ls_shortreads_species <- c(ls_shortreads_species, species_name)
}
...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    fn_tree <- paste0(dir_busco_tree, busco, "/", busco, "_aligned.fa.treefile.species")
    ...
    topology_ref <- ape::keep.tip(tre, ls_refseq_species)
    topology_ref$edge.length <- NULL

    # iterate over reference sequences
    for (ref in ls_refseq_species) {
