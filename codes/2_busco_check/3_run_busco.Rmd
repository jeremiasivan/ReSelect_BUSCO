...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        for (k in 1:length(ls_idx)) {
            ...
            if (item_len == 1) {
                read_out[[colname_mono]] <- paste(read_out[[colname_mono]], ls_ref_sp, sep= ";")
                len_mono <- len_mono + item_len
            } else if (item_len == max_len) {
                read_out[[colname_main]] <- paste(read_out[[colname_main]], paste(ls_ref_sp, collapse=","), sep= ";")
                len_main <- len_main + item_len
            } else {
                read_out[[colname_sub]] <- paste(read_out[[colname_sub]], paste(ls_ref_sp, collapse=","), sep= ";")
