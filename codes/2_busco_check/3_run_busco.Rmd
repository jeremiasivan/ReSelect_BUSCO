...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        for (i in 1:length(ls_taxa)) {
            ...
            for (j in (i+1):length(ls_taxa)) {
                ...
                if (is_monophyletic) {
                    ...
                    ls_ref_temp <- c(ls_ref_temp, ls_taxa[j])
                }
            }
        ...
        for (k in 1:length(ls_idx)) {
            ...
            # convert ID to species name
            ls_ref_sp <- sapply(ls_ref[[k]], function(x) {
                tempname <- unlist(strsplit(x, split="--"))[2]
                ls_species_name[tempname]
            })

            ls_ref_sp <- paste(ls_ref_sp, collapse=",")

            # update each column
            if (item_len == 1) {
                read_out[[colname_mono]] <- ifelse(is.null(read_out[[colname_mono]]), ls_ref_sp, paste(read_out[[colname_mono]], ls_ref_sp, sep= ";"))
                len_mono <- len_mono + item_len
            } else if (item_len == max_len) {
                read_out[[colname_main]] <- ifelse(is.null(read_out[[colname_main]]), ls_ref_sp, paste(read_out[[colname_main]], ls_ref_sp, sep= ";"))
                len_main <- len_main + item_len
            } else {
                read_out[[colname_sub]] <- ifelse(is.null(read_out[[colname_sub]]), ls_ref_sp, paste(read_out[[colname_sub]], ls_ref_sp, sep= ";"))
