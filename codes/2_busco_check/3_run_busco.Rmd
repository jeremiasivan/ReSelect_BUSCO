...
for (prefix in ls_prefix) {
    ...
    if (!all(file.exists(file_ref, file_input_vcf))) {
        log4r::warn(fn_logger, paste0("File not found: FASTA or VCF file for ", prefix, ". Skipped."))
        next
    }
...
data.table::fwrite(df_output, file=fn_summary, sep="/t", quote=F, row.names=F)
