...
for (ref in ls_refseq) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        # busco files
        dir_busco_ref <- paste0(dir_busco_ref, ref, "/run_", params$busco_lineage, "/busco_sequences/single_copy_busco_sequences/", busco)
        # file_busco_fas <- paste0(dir_busco_ref, ".fna")
        file_busco_gff <- paste0(dir_busco_ref, ".gff")
        ...
        # open BUSCO GFF file
        # df_gff <- data.table::fread(file_busco_gff, header=FALSE, select=1:9)
        # data.table::setnames(df_gff, c("seqname", "source", "feature", "start", "end", "score", "strand", "frame", "attribute"))

        # extract BUSCO
        f_extract_fasta_from_gff(file_fas, file_out, file_busco_gff, params$exe_gffread)
    }
...
for (prefix in ls_prefix) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        # busco file
        file_busco_gff <- paste0(dir_busco_ref, ref, "/run_", params$busco_lineage, "/busco_sequences/single_copy_busco_sequences/", busco, ".gff")
