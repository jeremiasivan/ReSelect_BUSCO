...
for (ref in ls_refseq) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        if (!all(file.exists(file_busco_fas, file_busco_gff))) {
            log4r::warn(fn_logger, paste0("File not found: FASTA or GFF file for ", ref, ". Skipped."))
            return(NULL)
        }
...
for (prefix in ls_prefix) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        if (!file.exists(file_busco_gff_out)) {
            f_write_log(fn_log=fn_log, msg=paste0("Error: ", busco, " GFF for ", ref, " not found. Skipped."))
            return(NULL)
        }
