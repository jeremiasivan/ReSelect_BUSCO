...
for (ref in ls_refseq) {
    ...
    df_output_temp <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
        ...
        if (!is.null(coordinates$errmsg)) {
            ...
            return(data.table::data.table(ref=ref, busco=busco, is_error=TRUE, identity=0))
        }
        ...
        if (!is.null(output$errmsg)) {
            ...
            if (!output$is_identical){
                ...
                if (output$score < min_ref_busco_identity) {
                    return(data.table::data.table(ref=ref, busco=busco, is_error=TRUE, identity=output$score))
                }
            }
        }

        return(data.table::data.table(ref=ref, busco=busco, is_error=FALSE, identity=output$score))
    }
