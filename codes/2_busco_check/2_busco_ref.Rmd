...
for (ref in ls_refseq) {
    # output file
    mid_ident <- ifelse (params$type == "pipeline", "pipeline", "coordinate")
    fn_busco_gff_all <- paste0(dir_busco_ref, ref, "/all_busco_", mid_ident, ".gff")
...
for (prefix in ls_prefix) {
    ...
    mid_ident <- ifelse (params$type == "pipeline", "pipeline", "coordinate")
    file_gff <- paste0(dir_busco_ref, readref[2], "/all_busco_", mid_ident, ".gff")
