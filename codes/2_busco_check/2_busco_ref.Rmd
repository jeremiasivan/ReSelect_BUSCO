        ...
        df_output_temp <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
            ...
            if (!output$is_identical){
                ...
                if (score >= min_ref_busco_identity) {
                    return(data.table::data.table(ref=ref, busco=busco, is_error=FALSE, identity=paste("Identity:", score)))
                }

                return(data.table::data.table(ref=ref, busco=busco, is_error=TRUE, identity=paste("Identity:", score)))
            }
