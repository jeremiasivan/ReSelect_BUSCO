...
for (ref in ls_refseq) {
    ...
    df_output_temp <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
        ...
        df_busco_fulltable_subset <- df_busco_fulltable[df_busco_fulltable$V1==busco,]
        coordinates <- list(seqname=unlist(strsplit(df_busco_fulltable_subset$V3[1], split=":"))[1],
                            start=df_busco_fulltable_subset$V4[1],
                            stop=df_busco_fulltable_subset$V5[1],
                            strand=df_busco_fulltable_subset$V6[1])
        ...
        f_write_log(fn_log=fn_log, paste0("Warn: ", busco, " FASTA file for ", ref, ". Extract Success."))

        # sanity check if FASTA is the same
        output <- f_compare_fasta(file_busco_fas, file_busco_concat)

        f_write_log(fn_log=fn_log, paste0("Warn: ", busco, " FASTA file for ", ref, ". Compare Success."))
