...
for (ref in ls_refseq) {
    ...
    df_output_temp <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
        ...
        if (!file.exists(file_busco_gff)) {
            log4r::warn(fn_logger, paste0("File not found: ", busco, " GFF file for ", ref, ". Skipped."))
        ...
        target_id <- stringr::str_extract(df1$attribute[1], "^Target_ID=.*\\;")
        ...
        if (nrow(df_metaeuk_gff_subset) == 0) {
            f_write_log(fn_log=fn_log, paste0("Warn: ", busco, " GFF entries for ", ref, " are not found on Metaeuk GFF. Skipped."))
            return(data.table::data.table(ref=ref, busco=busco, is_error=TRUE, identity=NA))
        } else if (nrow(df_metaeuk_gff_subset[df_metaeuk_gff_subset$feature=="gene"]) > 1) {
            f_write_log(fn_log=fn_log, paste0("Warn: ", busco, " GFF entries for ", ref, " have multiple gene entries on Metaeuk GFF. Skipped."))
            return(data.table::data.table(ref=ref, busco=busco, is_error=TRUE, identity=NA))
        }
        ...
        if (!output$is_identical){
            score <- as.numeric(output$score)
            if (score >= min_ref_busco_identity) {
                return(data.table::data.table(ref=ref, busco=busco, is_error=FALSE, identity=score))
            }
