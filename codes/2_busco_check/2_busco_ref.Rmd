...
} else if (params$type == "hybpiper") {
    dir_busco <- paste0(currentdir, "/busco_check/busco_hybpiper/")
    f_write_log(fn_log=fn_log, msg="Running HybPiper2-based method.")
} else {
...
fn_target_gene_hybpiper <- paste0(dir_busco_ref, "target_genes.fa")
...
for (read in 1:nrow(df_reads)) {
    ls_species_name[df_reads$id[read]] <- paste0("r_",df_reads$species[read])
}
```
...
# generate target genes for HybPiper
if (params$type == "hybpiper" && (!file.exists(fn_target_gene_hybpiper) || params$redo)) {
    for (ref in ls_refseq) {
        # iterate over BUSCO
        for (busco in shared_busco) {
            # input file
            file_busco_fasta <- paste0(dir_busco_ref, ref, "/run_", params$busco_lineage, "/busco_sequences/single_copy_busco_sequences/", busco, ".faa")
            f_fasta2msa(file_busco_fasta, paste0(ref,"-",busco), fn_target_gene_hybpiper)
        }
    }
}
```
...
} else if (params$type == "pipeline" || params$type == "hybpiper") {
    df_chr_busco <- data.table::data.table(ref=character(), busco=character(), is_error=logical(), note=character())

    # extract total number of BUSCO
    len_shared_busco <- len(shared_busco)
...
if (!is.null(df_mapped_reads_busco) && !params$redo) {
    ...
    if (params$type == "coordinate" || params$type == "" || is.null(params$type)) {
        ...
        knitr::knit_child(paste0(params$codedir,"/2_busco_check/2c_hybpiper.Rmd"))
    }
}
...
output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    if (params$type == "hybpiper") {
        for (read in ls_shortreads) {
            file_busco_read <- paste0(dir_busco_read, read, "/", busco, "/", read, "/sequences/FAA/", busco, ".FAA")
