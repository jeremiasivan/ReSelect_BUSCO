...
for (ref in ls_refseq) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        is_fasta_same <- f_compare_fasta(file_busco_fas, file_busco_concat)
        if (!is_fasta_same){
            log4r::warn(fn_logger, paste0("Warn: ", busco, " FASTA file for ", ref, ". Files are not the same."))
        }
    }
...
for (prefix in ls_prefix) {
    ...
    foreach (busco = shared_busco) %dopar% {
        ...
        read_depth <- f_calculate_read_depth(file_bam, file_busco_gff_out, params$exe_samtools)
        if (read_depth < 20) {
            log4r::warn(fn_logger, msg=paste("BUSCO excluded:", busco, "for", prefix, "with read depth of", read_depth))
            return(NULL)
        }
