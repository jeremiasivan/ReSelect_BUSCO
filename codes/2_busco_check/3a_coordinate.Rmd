...
for (prefix in ls_prefix) {
    ...
    output <- foreach (busco = shared_busco, .combine='c') %dopar% {
        ...
        # output directory
        dir_out_qualimap <- paste0(dir_busco_out, "/qualimap/", busco, "/")
        if (!dir.exists(dir_out_qualimap)) {
            dir.create(dir_out_qualimap, recursive=T)
        }
        ...
        read_coverage <- f_calculate_read_coverage(file_bam, file_busco_gff_out, file_busco_bed, params$exe_gff2bed, params$exe_samtools)
