...
fn_mntd_sum <- paste0(dir_busco, params$prefix, ".mntd.sum")
```
...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (i in 1:nrow(mntd_result)) {
        ...
        if (mntd_result$mntd.obs.z[i] > 0 && mntd_result$mntd.obs.p[i] > 0.95) {
            mntd_sum[[ls_species_name[ref_name]]] <- "cluster"
        } else if (mntd_result$mntd.obs.z[i] < 0 && mntd_result$mntd.obs.p[i] < 0.05) {
            mntd_sum[[ls_species_name[ref_name]]] <- "spread"
        } else {
...
df_mntd_colnames <- colnames(df_mntd_sum_output)
df_mntd_colnames <- df_mntd_colnames[df_mntd_colnames != "busco"]

# save summary file
f_write_log(fn_log=fn_mntd_sum, msg="---------- CLUSTER ----------")
for (ref in df_mntd_colnames) {
    ls_busco <- df_mntd_sum_output$busco[df_mntd_sum_output[[ref]] == "cluster"]
    f_write_log(fn_log=fn_mntd_sum, msg=c(ref, paste("-", ls_busco, "")))
}

f_write_log(fn_log=fn_mntd_sum, msg="----------  SPREAD ----------")
for (ref in df_mntd_colnames) {
    ls_busco <- df_mntd_sum_output$busco[df_mntd_sum_output[[ref]] == "spread"]
    f_write_log(fn_log=fn_mntd_sum, msg=c(ref, paste("-", ls_busco, "")))
}
```