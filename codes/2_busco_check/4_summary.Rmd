...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        plot <- ggplot(temp_df_dist, aes(x=ref_dist, y=read_dist, color=ls_colour_scheme_read[[read]])) +
        ...
        if (length(temp_read_out) != 0){
            plot <- plot + 
                ...
                stat_smooth(data=temp_df_no_out, geom='line', aes(x=ref_dist, y=read_dist, color=ls_colour_scheme_read[[read]], linewidth=4), method='lm', formula=y~x, alpha=0.5)
        }
