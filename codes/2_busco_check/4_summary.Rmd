...
df_mntd <- data.table::fread(df_mntd_sum_output)
...
for (i in 1:nrow(df_reads_subset)) {
    ...
    # update data.frame
    df_reads_ref <- rbind(df_reads_ref, c(species=species_name, reads=df_reads_subset$id[i], ref=ref_id))
}
...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (i in 1:nrow(df_reads_ref)) {
        ls_dist <- c(ls_dist, distance_tre[rownames(distance_tre)==df_reads_ref$reads[i],
