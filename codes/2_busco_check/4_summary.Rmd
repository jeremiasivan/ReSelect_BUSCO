...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        if (length(read_species_name) != 1){
            log4r::info(fn_logger, paste0("Error: multiple entries are found for read ", read, ". Skipped."))
            next
        }
        ...
        for (ref in ls_refseq) {
            ...
            ref_species_name <- df_reads_ref$species[df_reads_ref$ref==ref]
            if (length(ref_species_name) != 1){
                log4r::info(fn_logger, paste0("Error: multiple entries are found for ref ", ref, ". Skipped."))
                next
            }
            ...
            if (read_species_name == ref_species_name) {
                ...
                ref_id <- df_reads_ref$ref[df_reads_ref$species==read_species_name]
