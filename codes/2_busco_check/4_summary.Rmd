...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    print(ggplot(df_dist, aes(x=ref_dist, y=read_dist, color=read)) +
        ...
        geom_text(data=df_refs_outlier, aes(label=rownames(df_refs_outlier), size=20), vjust=-1.2, hjust=0.5) +
        geom_text(data=df_read_outlier, aes(label=rownames(df_read_outlier), size=20), vjust=-1.2, hjust=0.5) +
