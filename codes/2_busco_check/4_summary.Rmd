...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        df_reads_ref <- data.table::data.table(read=character(), ref=character(), ref_dist=numeric(), read_dist=numeric())
        ...
        for (ref in ls_refseq) {
            ...
            # update data.frame
            df_reads_ref <- rbind(df_reads_ref, data.table::data.table(read=ls_species_name[read],
                                                                       ref=ls_species_name[ref],
                                                                       ref_dist=ref_dist,
                                                                       read_dist=read_dist))
        }

        # run linear model
        lm_result <- lm(read_dist~ref_dist, data=df_reads_ref)
        read_out[[ls_species_name[read]]] <- paste0(round(summary(lm_result)$r.squared, 3),
                                                    " (", round(summary(lm_result)$coefficients[2,4], 3), ")")

        # update data.table
        df_dist <- rbind(df_dist, df_reads_ref)
    }
