...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    # calculate phylogenetic distance
    tre_original_dist <- ape::cophenetic.phylo(tre_original)
    ...
    for (read in ls_shortreads_available) {
        ...
        read_grouping <- df_reads$grouping[df_reads$id == read]
        ...
        temp_df_dist_filtered_noout <- temp_df_dist_filtered[temp_df_dist_filtered$grouping != "Out",]

        # run linear models
        lm_filtered_noout <- lm(read_dist~ref_dist, data=temp_df_dist_filtered_noout)
        read_out[[read_species_name]] <- paste0(coef(lm_filtered_noout)["read_dist"])
        ...
        plot <- ggplot(temp_df_dist, aes(x=ref_dist, y=read_dist, color=grouping)) +
            ...
            stat_smooth(data=temp_df_dist_filtered, geom='line', aes(x=ref_dist, y=read_dist, linewidth=4, linetype=2), method='lm', formula=y~x, alpha=0.5)  + 
            stat_smooth(data=temp_df_dist_filtered_noout, geom='line', aes(x=ref_dist, y=read_dist, linewidth=4, linetype=1), method='lm', formula=y~x) +
