...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        # run linear model
        lm_result <- robustbase::lmrob(read_dist~ref_dist, data=temp_df_dist)
    ...
    plot <- ggplot(df_dist, aes(x=ref_dist, y=read_dist, color=read)) +
        ...
        geom_smooth(method='robustbase::lmrob', formula= y~x, aes(linewidth=4)) +
        geom_text_repel(aes(label=ref), size=12) +
