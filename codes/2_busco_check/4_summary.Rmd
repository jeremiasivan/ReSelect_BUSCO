...
ls_output <- foreach (busco = shared_busco, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        # extract index
        idx_entry_read <- match(read, df_reads_ref$reads)
        if (is.na(idx_entry_read)){
            log4r::info(fn_logger, paste0("Error: no entry is found for ", read, ". Skipped."))
            next
        }
        ...
        for (ref in ls_refseq) {
            # extract index
            idx_entry_ref <- match(ref, df_reads_ref$ref)
            if (is.na(idx_entry_ref)){
                log4r::info(fn_logger, paste0("Error: no entry is found for ", ref, ". Skipped."))
                next
            }

            # calculate distance for reference
            ref_dist <- NULL
            if (idx_entry_read == idx_entry_ref) {
                ref_dist <- 0
            } else {
                ref_id <- df_reads_ref$ref[idx_entry_read]
                ref_dist <- distance_tre[rownames(distance_tre)==ref_id, colnames(distance_tre)==ref]
            }
            ...
            # update data.frame
            temp_df_dist <- rbind(temp_df_dist, data.table::data.table(read=df_reads_ref$species[idx_entry_read],
                                                                       ref=df_reads_ref$species[idx_entry_ref],
                                                                       ref_dist=ref_dist,
                                                                       read_dist=read_dist))
        }
