...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    # check if BUSCO is reference-only
    if (length(ls_shortreads_available) == 0) {
        return(NULL)
    }
    ...
    for (read in ls_shortreads_available) {
        ...
        # return NULL if table is of zero row
        if (nrow(temp_df_dist) == 0) {
            return (NULL)
        }

        # check if all values are NULL
        if (all(is.na(temp_df_dist$ref_dist)) || all(is.na(temp_df_dist$read_dist))) {
            return (NULL)
        }
        ...
        if (nrow(temp_df_dist_filtered) > 0) {
            # check if all values are NULL
            if (!all(is.na(temp_df_dist_filtered$ref_dist)) && !all(is.na(temp_df_dist_filtered$read_dist))) {
        ...
        if (nrow(temp_df_dist_filtered_noout) > 0) {
            # check if all values are NULL
            if (!all(is.na(temp_df_dist_filtered_noout$ref_dist)) && !all(is.na(temp_df_dist_filtered_noout$read_dist))) {
