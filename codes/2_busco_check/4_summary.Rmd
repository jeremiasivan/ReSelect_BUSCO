...
df_output <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
    ...
    # output variables
    read_out <- data.table::data.table(busco=character(), read=character(), lm_r2=numeric(), lm_slope=numeric(), lm_p=numeric(), spearman_rho=numeric(), spearman_p=numeric())
    ...
    for (read in ls_shortreads_available) {
        ...
        ls_refseq_available <- grep(paste0("^",read), ls_refseq, value=T)
