...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        if (length(temp_read_out) != 0) {
            ...
            if (all(!is.null(lm_stats)) && all(!is.null(lm_stats_no_out))) {
                if (is.na(lm_stats_no_out$pvalue)) {
                    for (ref in temp_df_out$ref) {
                        ref_out[[read]][[ref]] <- "?"
                    }

                    # extract reference outliers
                    ls_refseq_outs <- unique(temp_df_dist$ref)
                    ls_refseq_outs <- ls_refseq_outs[!ls_refseq_outs %in% temp_df_out$ref]
                    for (ref in ls_refseq_outs) {
                        ref_out[[read]][[ref]] <- is_problematic
                    }

                } else if ((lm_stats_no_out$pvalue >= 0.05 && is_problematic) || (lm_stats_no_out$pvalue <= 0.05 && !is_problematic)) {
                    for (ref in temp_df_out$ref) {
                        ref_out[[read]][[ref]] <- is_problematic
                    }
    ...
    if (nrow(df_read_outlier) > 0){
        plot <- plot + 
            ...
            stat_smooth(data=df_read_no_outlier, geom='line', aes(x=ref_dist, y=read_dist, color=read, linewidth=4), method='lm', formula=y~x, alpha=0.5)
    }
...
for (read in ls_shortreads) {
    # output file
    fn_temp_rdata <- paste0(fn_prefix, ".RData")
    fn_temp_out <- paste0(fn_prefix, ".", read, ".chr.sumtable")
