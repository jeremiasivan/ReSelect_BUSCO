...
# visualisation
plot <- ggplot(df_summary, aes(x=dist, fill=read)) +
...
df_busco_nrf <- foreach(busco = shared_busco, .combine='rbind') %dopar% {
    ...
    # calculate nRF
    nrf_phangorn <- f_calculate_nRF(fn_out_read_aligned_treefile, fn_out_refs_aligned_treefile)
...
plot <- ggplot(df_busco_nrf_long, aes(x=value, colour=name, fill=name, xmin=0, xmax=1)) +
    geom_histogram(binwidth=0.083, position="dodge", size=3, alpha=0.25) +
    facet_wrap(~name) +
...
```{r check-summary-hybpiper-astral}
...
ls_trees <- foreach (busco = shared_busco, .combine='c') %dopar% {
    # input file
    fn_tree <- paste0(dir_check_tree, busco, "/", busco, "_read_aligned.faa.treefile")
...
fn_outfile <- paste0(dir_check_tree, "alltrees.tre")
system(paste("cat", ls_trees, ">", fn_outfile))

# run ASTRAL-III
fn_astral_outfile <- paste0(dir_check_tree, "alltrees.astral.tre")
fn_astral_logfile <- paste0(dir_check_tree, "alltrees.astral.log")
...
f_write_log(fn_log=fn_log, msg=paste0("Species tree: ", nrf_dist, " (qScore: ", quartet_score, ")"))
