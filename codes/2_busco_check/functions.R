...
f_calculate_treedist <- function(fn_gene_tree, fn_refs_tree, min_bootstrap) {
    ...
    # open the trees
    gene_tree <- ifelse(is.character(typeof(fn_gene_tree)), ape::read.tree(fn_gene_tree), fn_gene_tree)
    refs_tree <- ifelse(is.character(typeof(fn_refs_tree)), ape::read.tree(fn_refs_tree), fn_refs_tree)
