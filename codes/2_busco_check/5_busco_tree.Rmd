...
for (i in 1:length(ls_refseq)) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        return(data.table::data.table(n_ref=i, busco=busco, ntaxa=length(tre$tip.label), nrf=nrf_dist, nrf_highbs=nrf_dist_high_bs, sum_genome_dist=genome_dist, ninfsites=n_infsites))
    }
...
plot <- ggplot(df_output, aes(x=sum_genome_dist, y=nrf)) +
    geom_point(aes(color=ninfsites), size=2, show.legend=F) +
...
plot <- ggplot(df_output, aes(x=sum_genome_dist, y=nrf_highbs)) +
    geom_point(aes(color=ninfsites), size=2) +
    ggtitle("Normalised RF Distance across Total Genomic Distances") + xlab("Genomic distance") + ylab("nRF distance") +
...
plot <- ggplot(df_output, aes(x=as.factor(n_ref), y=nrf, fill=ninfsites)) +
    geom_point(aes(color=ninfsites), position=position_jitter(height=0.01, width=0.05), size=2, show.legend=F) +
