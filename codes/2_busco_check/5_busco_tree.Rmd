...
if (file.exists(fn_busco_sumtable) && !params$redo) {
    df_output <- data.table::fread(fn_busco_sumtable)
} else {
    # create doSNOW cluster
    nwcl <- makeCluster(nthread)
    doSNOW::registerDoSNOW(nwcl)
...
# visualisation
plot <- ggplot(df_output, aes(x=as.factor(n_ref), y=dist, fill=as.factor(n_ref))) +
    geom_flat_violin(position=position_nudge(x=0.2), alpha=0.4) +
    geom_point(aes(color=as.factor(n_ref)), position=position_jitter(w=0.15), size=1, alpha=0.4, show.legend=F) +
