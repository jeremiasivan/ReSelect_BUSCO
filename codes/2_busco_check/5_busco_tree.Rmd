...
fn_busco_nref_tiff <- paste0(dir_check_tree, "nRF_nref.tiff")
fn_busco_nref_highbs_tiff <- paste0(dir_check_tree, "nRF_highbs_nref.tiff")
fn_busco_genomedist_tiff <- paste0(dir_check_tree, "nRF_genomedist.tiff")
fn_busco_genomedist_highbs_tiff <- paste0(dir_check_tree, "nRF_highbs_genomedist.tiff")
```
...
# output data.table
df_output <- data.table::data.table(n_ref=numeric(), busco=character(), ntaxa=numeric(), nrf=numeric(), nrf_highbs=numeric(), ninfbranch=numeric(), sum_genome_dist=numeric(), ninfsites=numeric())
...
for (i in 1:length(ls_refseq)) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        nrf_dist_high_bs <- f_extract_high_support_branch(fn_tree_one, fn_tree_two, 95)

        return(data.table::data.table(n_ref=i, busco=busco, ntaxa=length(tre$tip.label),
                                      nrf=nrf_dist, nrf_highbs=nrf_dist_high_bs$nrf, ninfbranch=nrf_dist_high_bs$n_infbranch,
...
plot <- ggplot(df_output, aes(x=sum_genome_dist, y=nrf)) +
    geom_point(size=2) +
...
plot <- ggplot(df_output, aes(x=sum_genome_dist, y=nrf_highbs)) +
    geom_point(aes(color=ninfbranch), size=2) +
    ggtitle("Normalised RF Distance across Total Genomic Distances") + xlab("Genomic distance") + ylab("nRF distance") +
...
plot <- ggplot(df_output, aes(x=as.factor(n_ref), y=nrf)) +
...
plot <- ggplot(df_output, aes(x=as.factor(n_ref), y=nrf_highbs)) +
    geom_point(aes(color=ninfbranch), position=position_jitter(height=0.01, width=0.05), size=2) +
    geom_violin(alpha=0.5) +
    ggtitle("Normalised RF Distance across Number of References") + xlab("Number of references") + ylab("nRF distance") +
    viridis::scale_color_viridis() +
    guides(size="none")
