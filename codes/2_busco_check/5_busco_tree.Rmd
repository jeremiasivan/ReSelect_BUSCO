...
# output data.table
df_output <- data.table::data.table(n_ref=numeric(), busco=character(), is_problematic=logical(), ntaxa=numeric(), dist=numeric())
...
for (i in 1:length(ls_refseq)) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        return(data.table::data.table(n_ref=i, busco=busco, is_problematic=FALSE, ntaxa=length(tre$tip.label), dist=nrf_dist))
    }
