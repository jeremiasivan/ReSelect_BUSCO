...
# output data.table
df_output <- data.table::data.table(n_ref=numeric(), busco=character(), ntaxa=numeric(), dist=numeric())
...
for (i in 1:length(ls_refseq)) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        genome_dist <- df_metadata %>%
            filter(busco=busco) %>%
            summarise(sum_dist=round(sum(genome_dist),3))

        # calculate nRF
        nrf_dist <- f_calculate_nRF(fn_tree_one, fn_tree_two)
        return(data.table::data.table(n_ref=i, busco=busco, ntaxa=length(tre$tip.label), dist=nrf_dist, sum_genome_dist=genome_dist$sum_dist))
    }
...
plot <- ggplot(df_output, aes(x=as.factor(n_ref), y=dist, fill=ntaxa)) +
    geom_point(aes(color=ntaxa), position=position_jitter(height=0.01, width=0.05), size=2, show.legend=F) +
...
plot <- ggplot(df_output, aes(x=sum_genome_dist, y=dist)) +
    geom_point(aes(color=ntaxa), size=2, show.legend=F) +
...
```{r check-busco-tree-cf}
f_write_log(fn_log=fn_log, msg=c("", "-- BUSCO Tree Concordance Factors --"))

# generate the species tree of each number of reference, run ASTRAL, and calculate qCF
