...
# output data.table
df_output <- data.table::data.table(n_ref=numeric(), busco=character(), ntaxa=numeric(), dist=numeric(), sum_genome_dist=numeric(), ninfsites=numeric())
...
for (i in 1:length(ls_refseq)) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        return(data.table::data.table(n_ref=i, busco=busco, ntaxa=length(tre$tip.label), dist=nrf_dist, sum_genome_dist=genome_dist, ninfsites=n_infsites))
    }
