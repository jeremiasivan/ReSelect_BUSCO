...
f_run_mntd(shared_busco_filtered, ls_refseq, TRUE, dir_busco_tree, ls_species_name, prefix, params$thread)

# run MNTD without reference sequence
prefix <- paste0(dir_check_summary_mntd_noref, params$prefix)
f_run_mntd(shared_busco_filtered, ls_refseq, FALSE, dir_busco_tree, ls_species_name, prefix, params$thread)
```
...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    for (read in ls_shortreads) {
        ...
        if (all(is.na(temp_df_dist$read_dist)) || all(is.na(temp_df_dist$ref_dist))) {
            log4r::info(fn_logger, paste("Error: temporary distance table for", read, "has all NA column(s). Skipped."))
            next
        }
