...
ls_output <- foreach (busco = shared_busco_filtered, .combine='c') %dopar% {
    ...
    # add outliers label
    if (nrow(df_refs_outlier) > 0){
        plot <- plot + geom_text_repel(data=df_refs_outlier, aes(label=df_refs_outlier$ref), size=12)
    }

    if (nrow(df_read_outlier) > 0) {
        plot <- plot + geom_text_repel(data=df_read_outlier, aes(label=rep("READ", nrow(df_read_outlier))), size=12)
    }
