...
foreach (prefix = ls_prefix, .combine='c') %dopar% {
    # create outdir
    dir_qualimap_output <- paste0(dir_readmap, prefix, "/qualimap/")
    if (!dir.exists(dir_qualimap_output)) {
        dir.create(dir_qualimap_output, recursive=T)
    }

    # check if output file exists
    file_qualimap_html <- paste0(dir_qualimap_output, "qualimapReport.html")
    if (file.exists(file_qualimap_html) && !params$redo) {
        return(NULL)
    }
