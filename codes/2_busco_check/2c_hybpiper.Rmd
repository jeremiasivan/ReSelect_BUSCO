...
if (params$redo) {
    ...
    for (read in ls_shortreads) {
        ...
        # log file
        file_busco_log <- list.files(dir_hybpiper_out, pattern=paste0("^", read, ".*log$"), recursive=F, full.names=F)
        finish_msg <- system(paste("grep 'Finished running.*hybpiper assemble*'", file_busco_log[length(file_busco_log)]), intern=T)
...
for (read in ls_shortreads) {
    ...
    df_output_temp <- foreach (busco = shared_busco, .combine='rbind') %dopar% {
        ...
        if (!file.exists(file_busco_fasta)) {
            return(data.table::data.table(ref=NA, read=read, busco=busco, is_error=TRUE, note=paste("File not found:", file_busco_fasta)))
        }
