...
df_output <- data.table::data.table(n_ref=numeric(), busco=character(), ntaxa=numeric(), n_spmatch=numeric(),
                                    nrf=numeric(), nrf2=numeric(), nrf2_highbs=numeric(),
...
for (i in 1:ngrouping) {
    ...
    df_output_subset <- foreach (busco = ls_uq_busco, .combine='rbind') %dopar% {
        ...
        nrf2 <- round((gene_tree_dist+refs_tree_dist) / nrf_denominator, 3)
        ...
        nrf2_highbs <- round((gene_tree_dist_highbs+refs_tree_dist_highbs) / nrf_denominator, 3)

        return(data.table::data.table(n_ref=i, busco=busco, ntaxa=length(tre$tip.label), n_spmatch=n_spmatch,
                                      nrf=nrf_dist, nrf2=nrf2$dist, nrf2_highbs=nrf2_highbs$dist,
                                      sum_genome_dist=genome_dist, read_meanbs=read_meanbs, read_infsites=n_infsites,
