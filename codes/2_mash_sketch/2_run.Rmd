...
ls_refseq_fasta <- sapply(ls_refseq, function(x) {
    ...
    file_fasta <- list.files(dir_fasta, pattern="*.fna", recursive=F, full.names=T)
...
for (i in ls_shortreads) {
    ...
    reads_species <- f_extract_species_from_reads(params$exe_esearch, params$exe_efetch, i)

    file_mashtab <- paste0(dir_shortreads, i, "/", i, ".mashtab")
    if (!file.exists(file_mashtab)) {
    ...
    refseq_species <- f_extract_species_from_assembly(params$exe_esearch, params$exe_efetch, params$exe_xtract, refseq_accession)
