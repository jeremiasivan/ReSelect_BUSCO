...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    for (j in 1:nrow(df_refs)) {
        ...
        df_specific_pairs <- rbind(df_specific_pairs, data.table::data.table(reads=NA, refs=NA, id=df_refs$id[j]))
    }
...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    ls_sptree_refs <- df_specific_pairs$id[is.na(df_specific_pairs$reads)]
    ls_sptree_read <- df_specific_pairs$id[!is.na(df_specific_pairs$reads)]
