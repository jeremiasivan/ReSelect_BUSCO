...
for (i in 1:nrow(df_sptree_metadata)) {
    # input files
    dir_scenario_output <- paste0(dir_sptree, df_sptree_metadata$scenario[i], "/")
    df_specific_pairs <- data.table::fread(paste0(dir_scenario_output, "metadata.tsv"))
    ...
    foreach(busco = shared_busco) %dopar% {
        ...
        if (!dir.exists(dir_scenario_alignment)) {
            dir.create(dir_scenario_alignment, recursive=T)
        }
        ...
        # iterate over prefix
        for (prefix in ls_sptree_read) {
            file_busco_read <- paste0(dir_busco_read, prefix, busco_read_dir_mid, busco, ".faa")
...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    # update list of BUSCO
    shared_busco <- list.dirs(paste0(dir_scenario_output, "alignment/"), full.names=F, recursive=F)
    ...
    ls_trees <- foreach (busco = shared_busco, .combine='c') %dopar% {
        ...
        fn_treefile_species <- paste0(dir_scenario_alignment, busco, "_aligned.faa.treefile.species")
    ...
    fn_outfile <- paste0(dir_scenario_output, "alltrees.tre")
    system(paste("cat", ls_trees, ">", fn_outfile))

    # run ASTRAL-III
    fn_astral_outfile <- paste0(dir_scenario_output, "alltrees.astral.tre")
    fn_astral_logfile <- paste0(dir_scenario_output, "alltrees.astral.log")
    f_astral(fn_outfile, fn_astral_outfile, fn_astral_logfile, params$exe_astral)
    
    # convert tips to species name
    fn_astral_sp_outfile <- paste0(dir_scenario_output, "alltrees.astral.sp.tre")
