...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    for (prefix in ls_sptree_read) {
        ...
        df_mapped_reads_update <- foreach(busco = shared_busco, .combine='rbind') %dopar% {
            # skip if the BUSCO entry is problematic
            df_subset_busco <- df_mapped_reads_busco_subset[df_mapped_reads_busco_subset$busco==busco,]
            if (nrow(df_subset_busco) != 1) {
                return(data.table::data.table(id=prefix, busco=busco, is_error=TRUE, note=paste("Invalid", busco, "entry for", prefix)))
            }

            # skip if BUSCO is not problematic
            if (!df_subset_busco$is_error[1]) {
