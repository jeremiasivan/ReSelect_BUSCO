...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    if (!dir.exists(dir_scenario_output)) {
        dir.create(dir_scenario_output, recursive=F)
    }
    ...
    ls_specific_pairs <- lapply(ls, function(x) {
    ...
    for (j in 1:nrow(df_refs$species)) {
        ...
        df_specific_pairs <- rbind(df_specific_pairs, data.table::data.table(reads=NULL, refs=NULL, id=df_refs$id[j]))
    }
...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    df_busco_metadata <- data.table(id=character(), busco=character(), is_error=logical(), note=character())

    # extract BUSCO from reference sequences
    ls_sptree_refs <- df_specific_pairs$id[is.null(df_specific_pairs$reads)]
    ls_sptree_read <- df_specific_pairs$id[!is.null(df_specific_pairs$reads)]
...
for (i in 1:nrow(df_sptree_metadata)) {
    # input files
    df_busco_metadata <- data.table::fread(paste0(dir_sptree, df_sptree_metadata$scenario[i], "/busco_metadata.tsv"))
    df_metadata <- data.table::fread(paste0(dir_sptree, df_sptree_metadata$scenario[i], "/metadata.tsv"))
...
for (i in 1:nrow(df_sptree_metadata)) {
    ...
    ls_trees <- foreach (busco = ls_uq_busco, .combine='c') %dopar% {
        # input file
        fn_tree <- paste0(dir_scenario_alignment, bbusco, "_aligned.faa.treefile")
