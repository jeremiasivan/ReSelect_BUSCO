# Source: https://raw.githubusercontent.com/Andy-B-123/MetaeukToGff3/main/MetaeukToGff3.py

#Update1: I'm aware more people are using this script and have commented out the troubleshooting library icecream which is not standard for Python/3.7.2 where I was developing this script :-)
#
#Convert the information in the header for the output file generated from MetaEuk to a gff3 format
#Requires python3
#Usage:
#	python MetaeukToGff3.py --MetaEuk_fas_output [output from metaeuk predict, required] --gff3_output_filename [desired output filename, default is input filename replacing suffix fas with gff3]
#The header provides two co-ordinates for each protein. I have provided both co-ordinates but checking manually it appears that the CDS annotation represents the output from MetaEuk the best.

import glob
import sys
#from icecream import ic 
from pathlib import Path
import argparse

# Initiate the parser
parser = argparse.ArgumentParser()

# Add long and short argument
parser.add_argument("--MetaEuk_fas_output", "-f", help="The MetaEuk fas output produced from MetaEuk easy-predict")
parser.add_argument("--gff3_output_filename", "-g", help="The output filename for the produced gff3 file. Default is the input filename with extension gff3",default='')

# Read arguments from the command line
args = parser.parse_args()

# store filenames
filename_infile=args.MetaEuk_fas_output
output_filename=args.gff3_output_filename

# Make output file
if output_filename == '':
	output_filename = filename_infile.replace("." + filename_infile.split(".")[-1], ".gff3")
ic(output_filename)


initial_list = []

with open(filename_infile) as infile:
	for line in infile:
		if line.startswith(">"):
			initial_list.append(line.strip(">").rstrip()) 	#remove the starting '>' for each fasta and trailing new-line character


with open(output_filename, 'w') as f:
	f.write("##gff_version 3\n")

	for entry in initial_list:
		entry_list = (entry.split("|",8)) 	#Use only the first 8 fields as they are constant. The last contains the the number of exon, which can vary
#		ic(entry_list)

		#below are the standard items for each entry
		entry_seqid = entry_list[1]
		entry_source = 'metaEuk_AndyGff'
		entry_score = entry_list[3]
		entry_strand = entry_list[2]
		entry_numExons = entry_list[5]
#		ic(entry_seqid)
#		ic(entry_source)
#		ic(entry_score)
#		ic(entry_strand)
#		ic(entry_numExons)

		#Now to set up each gff entry - starting with gene and mRNA, then the required number of exons/CDS 
		#Doing this a bit manually... Starting off with the easy case of + strand, then repeating with the - strand

		adjust_on_strand = 0
		if entry_strand == '+':
			adjust_on_strand = int(1)
	
			gene = [entry_seqid,entry_source,'gene',(int(entry_list[6]) +adjust_on_strand) ,(int(entry_list[7]) +adjust_on_strand),\
				entry_score,entry_strand,'.','ID=' + entry_list[0]]
#			ic(gene)
			for item in gene:
				f.write(str(item) + '\t')
			f.write('\n')

			mRNA = [entry_seqid,entry_source,'mRNA',(int(entry_list[6])+adjust_on_strand), (int(entry_list[7]) + +adjust_on_strand),\
				entry_score,entry_strand,'.','ID=' + entry_list[0] + '_mRNA;Parent='+entry_list[0]]
			for item in mRNA:
				f.write(str(item) + '\t')
			f.write('\n')

			
		
			#Exons and CDS - need a number of these for each entry
			exon_cds_segments = entry_list[-1].split('|')
#			ic(exon_cds_segments)
			exon_segments = []
			for gffLines in range(int(entry_numExons)):
				#exon will be the first number, CDS will be the second number mentioned
				#It seems that the second number provides better representation of the what the MetaEuk Protein output is (Eg translating CDS provides prot, translating exon doesn't
				exon_segments = []
#				ic(gffLines)
#				ic(exon_cds_segments[gffLines])
				exon_start=int(exon_cds_segments[gffLines].split('[')[0]) + +adjust_on_strand
#				ic(exon_start)
				exon_end=int(exon_cds_segments[gffLines].split(':')[1].split('[')[0]) + adjust_on_strand
#				ic(exon_end)
				exon_segments = [entry_seqid,entry_source,'exon',exon_start,exon_end,entry_score,entry_strand,'.',\
					'ID=' + entry_list[0] + '_exonA;Parent=' + entry_list[0] + '_mRNA']
				for item in exon_segments:
					f.write(str(item) + '\t')
				f.write('\n')

				cds_segments = []
#				ic(gffLines)
				cds_start=int(exon_cds_segments[gffLines].split(']')[0].split('[')[1]) + +adjust_on_strand
#				ic(cds_start)
				cds_end=int(exon_cds_segments[gffLines].split(':')[1].split('[')[1].strip(']')) + adjust_on_strand
#				ic(cds_end)
				cds_segments = [entry_seqid,entry_source,'CDS',cds_start,cds_end,entry_score,entry_strand,'.',\
					'ID=' + entry_list[0] + '_cdsB;Parent=' + entry_list[0] + '_mRNA']
				for item in cds_segments:
					f.write(str(item) + '\t')
				f.write('\n')

		elif entry_strand == '-':
			adjust_on_strand = int(1)
	
			gene = [entry_seqid,entry_source,'gene',(int(entry_list[6]) + adjust_on_strand),(int(entry_list[7]) +adjust_on_strand),\
				entry_score,entry_strand,'.','ID=' + entry_list[0]]
#			ic(gene)
			for item in gene:
				f.write(str(item) + '\t')
			f.write('\n')

			mRNA = [entry_seqid,entry_source,'mRNA',(int(entry_list[6])+adjust_on_strand), (int(entry_list[7]) +adjust_on_strand),\
				entry_score,entry_strand,'.','ID=' + entry_list[0] + '_mRNA;Parent='+entry_list[0]]
			for item in mRNA:
				f.write(str(item) + '\t')
			f.write('\n')

			
		
			#Exons and CDS - need a number of these for each entry
			exon_cds_segments = entry_list[-1].split('|')
#			ic(exon_cds_segments)
			exon_segments = []
			for gffLines in range(int(entry_numExons)):
				#exon will be the first number, CDS will be the second number mentioned
				#It seems that the second number provides better representation of the what the MetaEuk Protein output is (Eg translating CDS provides prot, translating exon doesn't
				exon_segments = []
#				ic(gffLines)
#				ic(exon_cds_segments[gffLines])
				exon_end=int(exon_cds_segments[gffLines].split('[')[0]) + +adjust_on_strand
#				ic(exon_start)
				exon_start=int(exon_cds_segments[gffLines].split(':')[1].split('[')[0]) + adjust_on_strand
#				ic(exon_end)
				exon_segments = [entry_seqid,entry_source,'exon',exon_start,exon_end,entry_score,entry_strand,'.',\
					'ID=' + entry_list[0] + '_exonA;Parent=' + entry_list[0] + '_mRNA']
				for item in exon_segments:
					f.write(str(item) + '\t')
				f.write('\n')

				cds_segments = []
#				ic(gffLines)
				cds_end=int(exon_cds_segments[gffLines].split(']')[0].split('[')[1]) + +adjust_on_strand
#				ic(cds_start)
				cds_start=int(exon_cds_segments[gffLines].split(':')[1].split('[')[1].strip(']')) + adjust_on_strand
#				ic(cds_end)
				cds_segments = [entry_seqid,entry_source,'CDS',cds_start,cds_end,entry_score,entry_strand,'.',\
					'ID=' + entry_list[0] + '_cdsB;Parent=' + entry_list[0] + '_mRNA']
				for item in cds_segments:
					f.write(str(item) + '\t')
				f.write('\n')

		else:
			print('No strand on this header? Major error')
	
