...
if (length(ls_shortread_qc) > 0 && !params$skip_shortreads_qc) {
    is_qcreads <- TRUE
}
```
...
foreach (read = ls_shortread_qc) %dopar% {
    ...
    fn_fastq_one <- paste0(dir_reads, reads_id, "_1.fastq")
    fn_fastq_two <- paste0(dir_reads, reads_id, "_2.fastq")
...
for (ref in ls_refseq) {
    ...
    dir_ref <- paste0(dir_refseq, ref, "/ncbi_dataset/data/", ref_id, "/")

    # check if there are multiple reference sequences
    file_ref <- list.files(dir_ref, pattern="*.fna$", recursive=F, full.names=T)
    if (length(file_ref) == 0) {
        log4r::warn(fn_logger, paste0("No reference is found for ", ref, ". Skipped."))
        next
    }
...
df_coverage <- foreach (prefix = ls_prefix, .combine='rbind') %dopar% {
    ...
    return(data.table::data.table(read_id=df_reads$id[df_reads$species==readref[1]],
                                  ref_id=df_refs$id[df_refs$species==readref[2]],
                                  read_sp=readref[1],
                                  ref_sp=readref[2],
