...
if (params$redo) {
    ...
    for (read in ls_shortreads) {
        ...
        file_fastq <- paste0(dir_shortreads_filtered, reads_id, ".pair1.truncated")
...
foreach (prefix = ls_readmap_run) %dopar% {
    ...
    # extract FASTQ file
    file_fastq_one <- paste0(dir_shortreads_filtered, reads_id, ".pair1.truncated")
    if (!file.exists(file_fastq_one)) {
        log4r::warn(fn_logger, paste0("File not found: filtered FASTQ file for ", read, ". Skipped."))
        return(NULL)
    }

    file_fastq_two <- paste0(dir_shortreads_filtered, reads_id, ".pair2.truncated")
